...
if (mode === 'payment') {
  if (!inputPath) {
    console.error('usage: research_signer.mjs payment <payment-required.json>');
    process.exit(1);
  }
  ...
  if (!inputPath) {
    console.error('usage: research_signer.mjs siwx <siwx-challenge.json>');
    process.exit(1);
  }
  ...
  // Legacy mode: first arg is a payment-required JSON path (backward compatible)
  if (!mode) {
    console.error('usage: research_signer.mjs <payment|siwx> <input.json>');
    console.error('       research_signer.mjs <payment-required.json>  (legacy)');
    process.exit(1);
  }
