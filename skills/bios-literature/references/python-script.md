...
This script uses the x402 v2 flow for BIOS literature search:
- request literature query (fast or deep mode)
...
- fast mode: results returned inline, no polling needed
- deep mode: authenticate poll requests via SIWX (SIWE wallet signatures)
- retry the request
- optionally poll until completion (deep mode)
...
Usage examples:
python3 literature.py --dry-run "What are the latest CRISPR therapies?"
...
python3 literature.py --mode deep --sources pubmed,arxiv "mRNA vaccine safety"
python3 literature.py --mode deep --no-poll "Search literature"
"""
...
SIGNER_JS = SCRIPT_DIR / "literature_signer.mjs"
...
def ensure_node_deps() -> None:
    ...
    if not pkg.exists():
        ...
        pkg.write_text(json.dumps({"name": "bios-literature-node-deps", "private": True}, indent=2))
...
def http_request(method: str, url: str, body: dict | bytes | None = None, headers: dict | None = None, content_type: str | None = None) -> tuple[int, dict, str]:
    data = None
    req_headers = {"User-Agent": "bios-literature-script/1.0"}
    if headers:
...
def start_literature(
    ...
    if per_source_limit is not None:
        body["perSourceLimit"] = per_source_limit
    if sources:
        body["sources"] = sources

    status, headers, text = http_request("POST", f"{BASE_URL}/api/agents/literature/query", body=body)
    if status == 200:
    ...
    status2, headers2, text2 = http_request(
        "POST",
        f"{BASE_URL}/api/agents/literature/query",
        body=body,
...
def fetch_status(job_id: str, payment_sig: str | None = None) -> tuple[int, dict]:
    url = f"{BASE_URL}/api/agents/literature/jobs/{job_id}"
...
def poll_until_done(job_id: str, interval: int, timeout_s: int) -> dict:
    started = time.time()
    print("Waiting 30s before first poll ...", file=sys.stderr)
    time.sleep(30)
    while time.time() - started < timeout_s:
        status, data = fetch_status(job_id)
        ...
        if status == 200 and state == "failed":
            print(f"Literature search failed. Data: {json.dumps(data.get('data'), indent=2)}", file=sys.stderr)
            raise RuntimeError("Literature search failed upstream")
        ...
        if status == 402:
            ...
            status2, data2 = fetch_status(job_id, payment_sig)
            if status2 == 200:
                return data2
...
def dry_run(question: str, mode: str = "fast") -> None:
    body: dict = {"question": question, "mode": mode}
    status, headers, text = http_request("POST", f"{BASE_URL}/api/agents/literature/query", body=body)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="BIOS Literature Search via x402 v2")
    parser.add_argument("query", help="Research question for the literature search")
    parser.add_argument("--mode", choices=["fast", "deep"], default="fast", help="Search mode (default: fast)")
    parser.add_argument("--sources", type=str, default=None, help="Comma-separated sources: arxiv,pubmed,clinical-trials")
    parser.add_argument("--max-results", type=int, default=None, help="Maximum total results")
    parser.add_argument("--per-source-limit", type=int, default=None, help="Maximum results per source")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--no-poll", action="store_true", help="Submit the paid request and stop (deep mode: returns jobId)")
    ...
    try:
        ...
        # Steps 1–2: Start literature query with payment
        result = start_literature(
            args.query,
            mode=args.mode,
        ...
        # Fast mode: results are already in the response
        if args.mode == "fast":
            return 0

        # Deep mode: need jobId for polling
        job_id = result.get("jobId")
        if not job_id:
            raise RuntimeError("No jobId returned for deep mode")

        if args.no_poll:
            return 0

        # Step 3: Poll until done (deep mode only)
        completed = poll_until_done(job_id, args.poll_interval, args.timeout)
