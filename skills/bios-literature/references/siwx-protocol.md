...
The GET endpoint for deep-mode literature results requires **SIWX (Sign-In With X)** authentication:
- `GET /api/agents/literature/jobs/{jobId}` — poll status / retrieve results

Only the wallet that paid for the literature search can access this endpoint. This uses [SIWE (EIP-4361)](https://eips.ethereum.org/EIPS/eip-4361) messages signed with `personal_sign` (EIP-191).
...
```json
{
  ...
  "siwx": {
    ...
    "uri": "https://x402.ai.bio.xyz/api/agents/literature/jobs/a1b2c3d4-e5f6-...",
...
5. Signer address matches the wallet that originally paid for the literature search

If any check fails, the server returns 401 (or 400 for malformed headers).

## Wallet requirement

The SIWX signer **must be the same wallet** that paid for the literature search in Step 2. The server stores the paying wallet address on the job record and compares it during verification.
