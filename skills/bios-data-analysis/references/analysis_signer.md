 ...
 *
 * Usage:
 *   node analysis_signer.mjs payment <payment-required.json>
 *   node analysis_signer.mjs siwx <siwx-challenge.json>
 *
 * Env:
...
if (mode === 'payment') {
  if (!inputPath) {
    console.error('usage: analysis_signer.mjs payment <payment-required.json>');
    process.exit(1);
  }
  ...
  if (!inputPath) {
    console.error('usage: analysis_signer.mjs siwx <siwx-challenge.json>');
    process.exit(1);
  }
  ...
  if (!mode) {
    console.error('usage: analysis_signer.mjs <payment|siwx> <input.json>');
    process.exit(1);
  }
